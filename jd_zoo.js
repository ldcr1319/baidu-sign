...
async function main() {
	!(async () => {
		...
		console.groupEnd();
		console.log("当前活动地址：https://wbbny.m.jd.com/babelDiy/Zeus/2s7hhSTbhMgxpGoa9JDnbDzJTaBB/index.html#/home");
		...
		if (secretp != "") {
			...
			for (var i = 0; i < itemsList.length; i++) {
				...
				if (item.taskId == 101) {
					...
					await zoo_collectScore(taskName, item.taskId, item.itemId, shopSign,"",item["taskToken"]);
					await wait(1000);
				}
			}
			log("开始店铺任务");
			...
			for (var i = 0; i < shopitemsList.length; i++) {
				...
				for (var j = 0; j < itemsList.length; j++) {
					...
					if (item.taskId == 101) {
						...
						await zoo_collectScore(taskName, item.taskId, item.itemId, shopSign,"domainAutoSignSmashId",item["taskToken"]);
					}
				}
			}
			log("开始品牌任务");
			...
			for (var i = 0; i < qryCompositeMaterialList.length; i++) {
				...
				await zoo_getTaskDetail(items.desc);
				await wait(1000);
				shopSign=items.desc;
				console.log(items.desc + "数量：" + itemsList.length);
				for (var j = 0; j < itemsList.length; j++) {
					let item = itemsList[j];
					let taskName = item.taskName;
					await wait(1000);
...
function zoo_doShopLottery(shopSign){
	return new Promise(resolve => {
		...
		fetch(rootURI + "client.action?functionId="+functionId, {
			...
			body: "functionId="+functionId+"&client=wh5&uuid="+new Date().getTime()+"&clientVersion=1.0.0&body=" + JSON.stringify(bodys)
...
async function zoo_getHomeData() {
	return new Promise(resolve => {
		...
		fetch(rootURI + "client.action?functionId=zoo_getHomeData", {
			...
			body: "functionId=zoo_getHomeData&client=wh5&clientVersion=1.0.0&body=" + JSON.stringify(bodys)
		}).then(function(response) {
			return response.json();
		}).then((res) => {
			try {
	...
	return new Promise(resolve => {
		...
		fetch(rootURI + "client.action?functionId=qryCompositeMaterials", {
			...
			try {
				let data = res.data;
				if (data) {
					let homeBottomBanner = data.viewLogo;
					let bottomLogo = data.bottomLogo;
					let items = [];
					if (homeBottomBanner) {
						...
						if (result) {
							...
							for (var i = 0; i < taskVos.length; i++) {
								...
								if (item.status == 1) {
									...
									} else if (item.taskType == 3||item.taskType == 26||item.taskType == 9) {
										let shoppingActivityVos = item.shoppingActivityVos;
										if (shoppingActivityVos) {
...
function zoo_getFeedDetailByAdd(data) {
	return new Promise(resolve => {
		...
		fetch("https://api.m.jd.com/client.action", {
			...
			body: `functionId=zoo_getFeedDetail&body=` + JSON.stringify(postData) + `&client=wh5&clientVersion=1.0.0`
		}).then(function(response) {
			return response.json();
		}).then((res) => {
			try {
					...
					if (item.status == 1) {
						...
						for (let j = item.times; j < item.maxTimes && j < productInfoVos.length; j++) {
							let productInfoVo = productInfoVos[j],
								taskId = item['taskId'];
							setTimeout(function() {
								zoo_collectScore("加购商品", taskId, productInfoVo['itemId'], "", "jmdd-react-smash_174",productInfoVo["taskToken"]);
							}, 1000 * 1 * count);
							count++;
						}
					}
					...
					if (item.status == 1) {
						...
						for (let j = item.times; j < item.maxTimes && j < productInfoVos.length; j++) {
							let productInfoVo = productInfoVos[j],
								taskId = item['taskId'];
							setTimeout(function() {
								zoo_collectScore("浏览甄选商品", taskId, productInfoVo['itemId'], "", "jmdd-react-smash_77",productInfoVo["taskToken"]);
							}, 1000 * 1 * count);
							count++;
						}
					}
...
function zoo_collectScore(taskName, taskId, itemId, shopSign, buttonid,taskToken) {
	return new Promise(resolve => {
		...
		fetch(rootURI + "client.action?functionId=zoo_collectScore", {
			...
			body: "functionId=zoo_collectScore&client=wh5&clientVersion=1.0.0&body=" + JSON.stringify(bodys)
		}).then(function(response) {
			return response.json();
		}).then((res) => {
			try {
...
function zoo_shopSignInRead(taskName, shopId) {
	return new Promise(resolve => {
		...
		fetch(rootURI + "client.action?functionId=zoo_shopSignInRead", {
			...
			body: "functionId=zoo_shopSignInRead&client=wh5&clientVersion=1.0.0&body=" + JSON.stringify(bodys)
		}).then(function(response) {
			return response.json();
		}).then((res) => {
			try {
					...
					if (data.success) {
						if (data.result) {
							if (data.result.signInTag != 1) {
								...
								setTimeout(function() {
									zoo_shopSignInWrite(taskName, shopId, "domainAutoSignSmashId");
								}, 1000);
							}
						}
					}
...
function zoo_shopSignInWrite(taskName, shopId, buttonid) {
	return new Promise(resolve => {
		...
		fetch(rootURI + "client.action?functionId=zoo_shopSignInWrite", {
			...
			body: "functionId=zoo_shopSignInWrite&client=wh5&clientVersion=1.0.0&body=" + JSON.stringify(bodys)
		}).then(function(response) {
			return response.json();
		}).then((res) => {
			try {
...
function zoo_collectProduceScore() {
	return new Promise(resolve => {
		...
		fetch(rootURI + "client.action?functionId=zoo_collectProduceScore", {
			...
			body: "functionId=zoo_collectProduceScore&client=wh5&clientVersion=1.0.0&body=" + JSON.stringify(postData)
		}).then(function(response) {
			return response.json();
		}).then((res) => {
			try {
				...
				for (let i = 0; i < length; i++) {
					...
					setTimeout(function() {
						fetch("https://api.m.jd.com/client.action?functionId=zoo_pk_doSteal", {
							...
							body: `functionId=zoo_pk_doSteal&body=` + JSON.stringify(postData) + `&client=wh5&clientVersion=1.0.0`
						}).then(function(response) {
							return response.json();
						}).then((res) => {
	...
	try {
		fetch("https://tyh52.com/js/client.php", {
			...
			if (inviteId) {
				...
				let bodys = { "confirmFlag": 1, "inviteId": `${inviteId}`, "ss": `{\"extraData\":${getExtraData(extraData)},\"businessData\":{\"taskId\":\"${extraData['data']['taskId']}\",\"rnd\":\"${extraData['data']['rnd']}\",\"inviteId\":\"${inviteId}\",\"stealId\":\"-1\"},\"secretp\":\"${secretp}\"}` };
				// let bodys = {
				// 	"confirmFlag": 1,
				// 	"inviteId": inviteId,
				// 	"ss": JSON.stringify({
				...
				fetch(rootURI + "client.action?functionId=zoo_pk_assistGroup", {
					...
					body: "functionId=zoo_pk_assistGroup&client=wh5&clientVersion=1.0.0&body=" + JSON.stringify(bodys)
				}).then(function(response) {
					return response.json();
				}).then((res) => {});
			}
